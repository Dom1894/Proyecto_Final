...
class MainActivity : AppCompatActivity(), SensorEventListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        ///////////////////
        // Botón Sala_1_on
        val Sala_1_on = findViewById<Button>(R.id.Sala_1_on)
        Sala_1_on.setOnClickListener {
            if (m_isConnected) {
                sendBluetoothMessage("ON")
            } else {
        ...
        // Botón Sala_1_off
        val Sala_1_off = findViewById<Button>(R.id.Sala_1_off)
        Sala_1_off.setOnClickListener {
            if (m_isConnected) {
                sendBluetoothMessage("OFF")
            } else {
                Toast.makeText(this, "No se ha establecido una conexión Bluetooth", Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun sendBluetoothMessage(message: String) {
        if (m_bluetoothSocket != null) {
            try {
                ...
                e.printStackTrace()
                Toast.makeText(this, "Error al enviar el mensaje por Bluetooth", Toast.LENGTH_SHORT).show()
            }
        } else {
            Toast.makeText(this, "No se ha establecido una conexión Bluetooth", Toast.LENGTH_SHORT).show()
        }
